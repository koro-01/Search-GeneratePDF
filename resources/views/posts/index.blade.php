...
<body class="bg-gray-100">
    <div class="container mx-auto px-4 py-8">
        ...
        <!-- Search Form and Download PDF Button -->
        <div class="mb-8 flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0 sm:space-x-4">
            <form action="{{ route('posts.search') }}" method="GET" class="flex items-center space-x-4 flex-grow">
                <input 
                    type="text" 
                    name="keyword" 
                    placeholder="search" 
            ...
            <a 
                href="{{ route('posts.pdf', ['keyword' => request('keyword')]) }}" 
                class="px-6 py-2 bg-green-500 text-white font-semibold rounded-md hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
            >
                Download PDF
            </a>
        </div>
